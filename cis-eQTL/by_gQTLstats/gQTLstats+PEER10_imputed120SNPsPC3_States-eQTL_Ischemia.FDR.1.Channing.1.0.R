#!/usr/bin/Rscript
message("############################################################################################################")
message("## R-script to run PEER10 & gQTLstats on Transcribe118 RNA-Seq for Ischemia cis-eQTL analyses with 3 PCs  ##")
message("##                                                                                                        ##")
message("##  Author: Dr. Alex Tzuu-Wang Chang                                                                      ##")
message("##    Date: 2016-09-27                                                                                    ##")
message("##                                                                                                        ##")
message("## Version: 1.0 For re-running of 2016-09-08 State cis-eQTL with 3 PCs                                    ##")
message("##                                                                                                        ##") 
...
message("###############################################################################")
print(Sys.time())
setwd("/path/analyses/retwc/20160927reRUN02")
...
if (length(args)==0) {
  ...
  exprPre    <- read.csv(args[1], header = TRUE, check.names = FALSE, stringsAsFactors = FALSE)
  exprPost   <- read.csv(args[2], header = TRUE, check.names = FALSE, stringsAsFactors = FALSE)
  phenotypes <- read.csv(args[3], header = TRUE, check.names = FALSE, stringsAsFactors = FALSE)
}
# system("ls x*")
...
# phenotypes <- read.csv("./InputFiles/CovariancesAll.csv", header = TRUE, check.names = FALSE, stringsAsFactors = FALSE)                                       
# GenoCov120 <- read.table("./InputFiles/Transcribe118_AllChr22_imputed_MAF0.15passed.ProPC.coord",header = TRUE,stringsAsFactors = FALSE)
